...
class Category(models.Model):
    name = models.CharField(
        verbose_name="Наименование категории", max_length=200
    )
    slug = models.SlugField(
        verbose_name="URL-адрес", max_length=50, unique=True
    )
...
class Title(models.Model):
    name = models.CharField(
        verbose_name="Наименование произведения", max_length=100, db_index=True
    )

    year = models.IntegerField(
        verbose_name="Год выпуска",
        validators=[MinValueValidator(0), MaxValueValidator(dt.now().year)],
    )
